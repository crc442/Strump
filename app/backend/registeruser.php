...
if ($res['cnt'] == 0) {
	$query = "insert into users (user_id ,user_first_name,user_last_name,dob,email_id,
		user_city,reputation,last_accessed_date,registration_date,
		password,status,isLoggedin) values
('$name','$fname','$lname','$dob','$emailID','$userCity',0,now(),now(),
	'$password','ACTIVE','Y')";
...
if ($qry_res) {
	...
	$arr = array('msg' => "", 'error' => 'User Already exists with same username');
}
//     $jsn = json_encode($arr);
//     print_r($jsn);
